...
U N O P T I M I Z E D   A N D  R E G I S T E R E D  C O D E  O F

Title of Paper  : Bezier Search Differential Evolution Algorithm (  BeSD )
SubTitle            : A comparative study with CEC2014's Competitors ; CRMLSP, MVO, WA, SHADE and LSHADE
...
(c) CIVICIOGLU , BESDOK  ( 15-JULY-2020 )
...
% Initialization
K = 5 ;   %  LINE 2
S = rand( K * N , D ) * ( up - low ) + low ;        %  LINE 2
fitS = feval( fnc , S , mydata ) ;                            %  LINE 3
...
for epk = 1 : maxcycle    %  LINE 5
   ...
   for i = 1 : N
      for index = 1 : D
         if T(i,index) < low, T(i,index) = rand * ( up - low ) + low ; end
         if T(i,index) > up,  T(i,index) = rand * ( up - low ) + low ; end
      end
   end
   
   %  Update the sub-pattern matrix, P and fitP  ;      %  LINES 34 - 35
   fitT = feval(fnc , T ,  mydata) ;
   ind = fitT < fitP ;
   P( ind , :  ) = T( ind , : ) ;
   fitP( ind )  = fitT( ind ) ;
